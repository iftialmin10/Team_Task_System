...
const selectClass =
  "min-h-11 min-w-0 rounded-lg border bg-white px-3 text-sm text-ink hover:border-slate-400 active:bg-slate-50";
...
const sortOptions: Array<{
  value: `${WorkItemQuery["sort"]}:${WorkItemQuery["order"]}`;
  label: string;
}> = [
...
function FilterFields({
  ...
  state: BrowseState;
  users: User[];
  onChange: (changes: Partial<BrowseState>) => void;
  idPrefix?: string;
}) {
  return (
    <>
      <label className="grid min-w-0 gap-1.5 text-xs font-semibold text-muted">
        ...
        <select
          ...
          value={state.owner ?? ""}
          onChange={(event) =>
            onChange({ owner: event.target.value || undefined })
          }
        >
          ...
          onChange={(event) =>
            onChange({
              status: (event.target.value ||
                undefined) as BrowseState["status"],
            })
          }
        >
          ...
          onChange={(event) =>
            onChange({
              priority: (event.target.value ||
                undefined) as BrowseState["priority"],
            })
          }
        >
      ...
      <label className="grid min-w-0 gap-1.5 text-xs font-semibold text-muted">
        ...
        <select
          ...
          onChange={(event) =>
            onChange({
              due: (event.target.value || undefined) as BrowseState["due"],
            })
          }
        >
    ...
    <div className="mt-7">
      <div className="flex gap-3">
        <label className="relative min-w-0 flex-1">
          ...
          <input
            ...
            className="min-h-11 w-full rounded-lg border bg-white py-2 pl-10 pr-3 text-sm shadow-subtle placeholder:text-slate-400 hover:border-slate-400"
          />
        </label>
        <button
          type="button"
        ...
        >
          Filters
          {activeCount > 0 && (
            <span className="ml-2 inline-flex min-w-5 justify-center rounded-full bg-blue-100 px-1.5 text-xs text-blue-800">
              {activeCount}
            </span>
          )}
        </button>
      ...
      <div className="mt-4 hidden grid-cols-5 gap-3 md:grid">
        <FilterFields
          state={state}
          users={users}
          onChange={(changes) => onChange(changes, { resetPage: true })}
        />
      ...
      <ActiveFilters
        state={state}
        users={users}
        onRemove={(key) => onChange({ [key]: undefined }, { resetPage: true })}
        onClear={onClear}
      />
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="min-h-11 rounded-lg px-3 text-sm font-semibold hover:bg-slate-100 active:bg-slate-200"
              >
                Close
              </button>
            </div>
            ...
            <div className="sticky bottom-0 mt-6 flex gap-3 border-t bg-white pt-4">
              <button
                ...
                className="min-h-11 flex-1 rounded-lg border px-4 text-sm font-semibold hover:bg-slate-50 active:bg-slate-100"
              >
                Reset
              </button>
              <button
                ...
                className="min-h-11 flex-1 rounded-lg bg-primary px-4 text-sm font-semibold text-white hover:bg-blue-800 active:bg-blue-900"
              >
                Apply filters
              </button>
            </div>
...
function ActiveFilters({
  ...
  state: BrowseState;
  users: User[];
  onRemove: (key: "search" | (typeof filterKeys)[number]) => void;
  onClear: () => void;
}) {
  const filters: Array<{
    key: "search" | (typeof filterKeys)[number];
    label: string;
  }> = [];
  if (state.search)
    ...
    filters.push({
      ...
      label:
        state.owner === "unassigned"
          ? "Owner: Unassigned"
          : `Owner: ${users.find(({ id }) => id === state.owner)?.name ?? state.owner}`,
    });
  if (state.status)
    filters.push({
      key: "status",
  ...
  if (state.priority)
    filters.push({
      key: "priority",
      label: `Priority: ${priorityLabels[state.priority]}`,
    });
  if (state.due)
    filters.push({ key: "due", label: `Due: ${dueLabels[state.due]}` });
